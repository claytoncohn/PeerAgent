...
chat_window_URL = "http://127.0.0.1:7860",
computational_model_state = C2STEMState()
agent = Agent(use_gui=True)

async def initialize_agent_server(websocket):
    """
    Initializes the agent server and sends the chat window URL to the client.

    This function initializes the WebSocket connection for the agent server. 
    It sends the predefined `chat_window_URL` to the client and invokes the agent's
    `talk` method. It also handles exceptions during initialization.
...
async def handler(websocket):
    ...
    2. Initializes the agent server for the connection.
